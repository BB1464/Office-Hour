dat <- structure(list(
  ...
  resp = c(
    ...
    0.3, 0.66, 0.57, 0.23, 0.31, 0.48, 0.5, 0.49,
...
# Looping through the response variable
vars = names(dat)[2:4]

vars

# List the function by names without using a formula
...
dat <- mtcars |> group_by(cyl) |>
  nest() |>
  mutate(model=map(.x = data,.f = ~lm(formula = mpg~.,data = .)),tidied=map(.x = model,.f = glance)) |>
  unnest(tidied)


dat %>%
  ggplot(aes(factor(cyl),r.squared, fill=cyl))+
  geom_bar(stat = "identity", show.legend = FALSE)



# Third Appraoch
# Running a linear model on separate group
library(tidyverse)
library(broom)
